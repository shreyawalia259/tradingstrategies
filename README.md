...
**Notes: Needed to brew install ta-lib first and then pip install it. 
...
Libraries: hvplot
Notes: having trouble displaying the plots with returns and sharpe ratio plot only showing for a second. Final dashboard is displayed just fine though. Code in dashboard.py file.
...
Note: for selenium you need to get the library, get chrome driver and all of that so it is easier to just use bs4 which can be seen in scraping.py
...
Note: Code can be found in file valuation.py
